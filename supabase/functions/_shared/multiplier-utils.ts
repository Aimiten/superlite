/**
 * Shared multiplier utilities for edge functions
 */
...
/**
 * Validates custom multipliers (server-side copy of frontend validation)
 */
...
export function shouldUseManualMultipliers(
  ...
  if (method !== 'manual') {
    console.log(`Not using manual multipliers: method is '${method}', not 'manual'`);
    return false;
  }
  
  const validation = validateCustomMultipliers(customMultipliers);
  if (!validation.isValid) {
    console.log(`Manual multipliers validation failed: ${validation.error}`);
    return false;
  }

  console.log('Using manual multipliers:', customMultipliers);
  return true;
}